...
export default function TranscactionsPage() {
  ...
  const fetchTxData = async () => {
    try {
      ...
      let response = await API.get(
        `/${chainId}/events/topics/${topic}/?starting-block=${start}&ending-block=${end}&sender-address=${contractAdd}&key=${COVALENT_KEY}`
      );
  ...
  return (
    <>
      ...
      {toggleModal && <EventModal isOpen={toggleModal} decodedData={decodedData} setToggleModal={setToggleModal} />}
      ...
      <div className="overflow-x-auto w-full   lg:flex lg:justify-center lg:items-end lg:flex-col">
        {resError === true ? (
          <div className="alert alert-error flex justify-center shadow-lg">Error: Something is not right</div>
        ) : null}
        ...
        <table className={`table w-full  table-normal`}>
          ...
          <tbody>
            {events?.length > 0 &&
              events.map(
                ({ tx_hash, tx_offset, log_offset, sender_address, block_height, block_signed_at, decoded }, index) => {
                  return (
                    <React.Fragment key={index}>
                      <tr>
                        ...
                        <td>{block_height}</td>
                        <td>
                          <span className="badge border-none badge-lg bg-slate-400 text-black">{tx_offset}</span>
                        </td>
                        <td>
                          <span className={`badge border-none badge-lg bg-slate-400 text-black`}>{log_offset}</span>
                        </td>
